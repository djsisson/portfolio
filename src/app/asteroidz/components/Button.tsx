...
import { useGameObject, useGameState, useGameStateDispatch } from "./Context";
			...
			case "Shop": {
				...
				if (quantity === x.maxQty) {
					...
					textToDisplay = `${x.name} (${quantity}) Cost: ${
						x.cost * x.multiplier ** quantity
					}`;
					toolTip = [
						`${x.name}`,
						`Cost: ${x.cost * x.multiplier ** quantity}`,
						`${x.requiredItems.map((x) => x.description).join(", ")}`,
					];
				}
				break;
	...
	useEffect(() => {
		...
		const hasItems = (x: Research | Upgrade | ShopItem) => {
			...
			x.requiredItems.forEach((item) => {
				const invenItem = inven.find((x) => x.name === item.required);
		...
		switch (type) {
			...
			case "Shop": {
				const x = _gameObjects.shopItems.find((x) => x.name === name);
				if (!x) return;
				calcCost = x.cost * x.multiplier ** quantity;
